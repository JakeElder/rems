...
const MapPickerComponent = ({ value: val, ...props }) => {
    const { config, isLoading } = (0, use_plugin_config_1.default)();
    const value = val === "null" ? undefined : val;
    return config && config.apiKey && config.defaultCenter ? (react_1.default.createElement(MapPickerInput_1.default, { value: value, ...props, config: config })) : (!isLoading && !config.apiKey) ||
        (!isLoading && !config.defaultCenter) ? (react_1.default.createElement(react_1.default.Fragment, null,
